...
`matches_term()` is word-boundary based and case-sensitive: it matches
`peerCwdFilter` for the query `peerCwdFilter`, but not for `Cwd` and not
for `peercwdfilter`. Fall back to `lower(content) LIKE '%needle%'` when
you need substring or case-insensitive recall — wider, noisier, no index. Rows with `message_type = 'usage'` carry empty content
